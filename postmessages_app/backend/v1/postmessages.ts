...
let server = http.createServer((req, res) => {
    ...
    // Data event is fired every time a new body. Chunk is sent by the client
    req.on("data", (chunk) => {
        body = body + chunk;
    });


    // The "end" event is fired when client's request is complete. We can now proceed with the response
    ...
    req.on("end", () => {
        ...
        if(req.url == "/api/v1/" && req.method=="GET") {
            ...
            try {
                ...
                if(isMessage(recv_message)) {
                    messages.push(recv_message);
                    ostream.write(JSON.stringify(recv_message) + "\n", 'utf8', () => {
            ...
            let parsed_query = url.parse(req.url, true /* true = parse query string*/).query;
            console.log(" Query: ".red + JSON.stringify(parsed_query));

            let query: string = <string>(parsed_query.index);
            let query_idx: number = parseInt(query);

            if(query_idx < messages.length) {
                messages[query_idx] = messages[messages.length - 1];
                messages.pop();
            ...
            // Secret endpoint, just to demonstrate why non-blocking functions are crucial in Node.JS
            ...
            */
            ...
            send_response( 404, { error:true, errormessage:"Invalid endpoint/method"} );
        }
    });
});
