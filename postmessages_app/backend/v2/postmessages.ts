...
let server = http.createServer( function( req, res ) {
    ...
    req.on("data", function( chunk ) {
        ...
        if( req.url == "/api/v2/" && req.method=="GET") {
            ...
            .then( (taglist) => {
                return send_response( 200, taglist );
            }).catch( (reason)=>{
                return send_response(404, { error: true, errormessage: "DB error"+reason });
            });
            ...
            message.getModel().findByIdAndDelete(query.id)
            .then( ()=> {
...
NOTE2: try..catch works well with synchronous code. With asynchronous code
       you must use await for asynchronous functions that might throw an exception.
    ...
    If f() is synchronous it works as expected. If f() is asynchronous
    the exception is lost! (f terminates correctly returning a Promise, the
    exception is thrown during the execution of the asynchronous code).
